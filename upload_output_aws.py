...
for file in glob.glob(raw_data_folder+"/*"):
    new_file_name = file.replace(args.dir,"output_ITE_autoware_universe_aws")
    s3.Bucket(s3_bucket_name).upload_file(Filename=file, Key=new_file_name)
    os.remove(file) # delete the file after uploading

# Upload rosbag
rosbag_folder = f"{log_dir}/rosbag_{args.nth}"
for file in glob.glob(rosbag_folder+"/*"):
    new_file_name = file.replace(args.dir,"output_ITE_autoware_universe_aws")
    s3.Bucket(s3_bucket_name).upload_file(Filename=file, Key=new_file_name)
    os.remove(file) # delete the file after uploading

# Upload rosbag
rosbag_folder = f"{log_dir}/cav_context_{args.nth}"
for file in glob.glob(rosbag_folder+"/*"):
    new_file_name = file.replace(args.dir,"output_ITE_autoware_universe_aws")
    s3.Bucket(s3_bucket_name).upload_file(Filename=file, Key=new_file_name)
    os.remove(file) # delete the file after uploading
