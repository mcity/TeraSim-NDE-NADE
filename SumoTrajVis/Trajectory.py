...
class PotentialIntersection:
    ...
    def __init__(self, duration, start_time=0, end_time=0, final_state_json=None, maneuvers_json=None) -> None:
        ...
        # if self.does_file_exist(self.final_state_json):
        #     self._read_from_file(self.final_state_json, self.maneuvers_json)
    ...
    # def _read_from_file(self, final_state_json, maneuvers_json):
    #     try:
    #         if self.does_file_exist(maneuvers_json):
    #             maneuvers = json.load(open(maneuvers_json, "r"))
    #             for veh_id, maneuver_challenge_moments in maneuvers.items():
    #                 self.intersections[veh_id]["intersection"] = maneuver_challenge_moments
    #         final_state = json.load(open(final_state_json, "r"))
    #         crash_1_id = final_state["veh_1_id"]
    #         crash_2_id = final_state["veh_2_id"]
...
class Trajectories:
    ...
    def __init__(self, file=None):
        ...
        self.timestep = None
        if file is not None:
            if file.endswith("fcd-output.xml") or file.endswith("fcd.xml") or file.endswith("fcd_all.xml"):
    ...
    def plot_points(self, time, ax=None, animate_color=False, lanes=None, intersection=None):
        """
        ...
        for traj in self.trajectories:
            ...
            if intersection.is_highlighted(traj.id, time):
                traj.point_plot_kwargs["color"] = "red"
