...
#     ego_veh_heading = ego_veh_obs['heading'] if ego_veh_obs else None
#     lead_veh_heading = lead_veh_obs['heading'] if lead_veh_obs else None

#     # same lane id
#     if (ego_veh_obs and lead_veh_obs):
#         ego_veh_pos = np.array([ego_veh_obs['position'][0], ego_veh_obs['position'][1]])
#         lead_veh_pos = np.array([lead_veh_obs['position'][0], lead_veh_obs['position'][1]])
#         relative_pos = lead_veh_pos - ego_veh_pos
#         relative_heading = get_sumo_angle(np.arctan2(relative_pos[1], relative_pos[0]) * 180 / np.pi)
#         if (ego_veh_obs['veh_id'].split(".")[0] == lead_veh_obs['veh_id'].split(".")[0]) or (ego_veh_lane_id == lead_veh_lane_id):
#             return True
#         elif abs(ego_veh_heading - lead_veh_heading) < 45 and abs(relative_heading - ego_veh_heading) < 20: # 20 degree to identify rear end
#             return True
#         else:
#             return False
#     else:
...
def get_collision_type_and_prob(observation, negligence_mode, location_region, neg_location_region):
    ...
    highway_cutin_prob = 7.2e-8 * 0.952 * 0.91 * 1.5 * 0.67 * 0.91 # some sideswipe, some angle
    highway_rearend_prob = 8.4e-5 * 0.748 * 0.46 * 1.5 * 1.3 * 1.5 * 0.71 * 0.77

    roundabout_rearend_prob = 3.6e-8 * 0.952 * 0.86 * 0.98 * 1.27
    roundabout_neglect_conflict_lead_prob = 3.6e-7 * 0.5 * 2.27 * 1.5 * 0.6 # ignore conflict
    roundabout_cutin_prob = 8.4e-8 * 1.121 * 0.91 * 1.2 * 0.93 # mostly sideswipe

    intersection_rearend_prob = 6e-7 * 0.85 * 1.14 * 0.888 * 1.05 * 1.03
    intersection_cutin_prob = 1.2e-5 * 0.804 * 1.31 * 0.888 * 1.05 * 0.6 # mostly sideswipe
    intersection_neglect_conflict_lead_prob = 1.2e-6 * 0.5 * 0.1 * 0.888 * 1.05 * 1.27 # ignore conflict
    intersection_tfl_prob = 1.2e-5 * 1.026 * 2.02 * 0.888 * 1.05 * 1.16 # ignore traffic light
    intersection_headon_prob = 1.2e-7 * 1.388 * 0.888 * 1.05 * 0.74 # head on
