...
class EnvMonitor:
    ...
    def save_observation(self, veh1_id, veh2_id):
        ...
        with open(
            os.path.join(f"{self.log_dir}/{self.exp_id}", "monitor.json"), "w"
        ) as f:
    ...
    def _filter_obs(self, veh_id, veh_observation):
        ...
        for key in veh_observation.keys():
            if key != "Ego" and veh_observation[key] is not None:
                observation.update({key: veh_observation[key]})
        return observation
    ...
    def update_vehicle_mode(self, control_cmds, importance_sampling_weight):
        ...
        for veh_id, control_cmd in control_cmds.items():
            if control_cmd and "mode" in control_cmd:
                ...
                negligence_type = (
                    control_cmd["info"].get("negligence_mode")
                    if "info" in control_cmd
                    and "negligence_mode" in control_cmd["info"]
                    else None
                )
                mode_info = {
                    ...
                    "time": utils.get_time(),
                    "lead_veh": (
                        obs_dict["local"]["Lead"]["veh_id"]
                        if obs_dict["local"]["Lead"]
                        else None
                    ),
                }
                if control_cmd["mode"] == "negligence":
                    ...
                    self.accept_collision_mode_record_trimmed.update(
                        {veh_id: mode_info}
                    )

    def add_maneuver_challenges(
        self,
        maneuver_challenge_dict,
        ...
        for veh_id, maneuver_challenge in maneuver_challenge_dict.items():
            if maneuver_challenge.get("negligence", 0) == 1:
                ...
                record_info = {
                    ...
                    "maneuver_challenge_avoidance": maneuver_challenge_avoidance_dict.get(
                        veh_id, None
                    ),
                    "maneuver_challenge_info": maneuver_challenge_info.get(
                        veh_id, None
                    ),
                }
    ...
    def export_final_state(self, veh_1_id, veh_2_id, final_state_log, end_reason):
        ...
        if veh_1_id is not None and veh_2_id is not None:
            ...
            avoid_car, avoid_mode, avoid_info, avoid_time = max(
                (
                    (vehicle, *data[(vehicle, "avoid_collision")])
                    for vehicle in vehicles
                ),
                key=lambda x: x[3],
            ...
            accept_car, accept_mode, accept_info, accept_time = max(
                (
                    (vehicle, *data[(vehicle, "accept_collision")])
                    for vehicle in vehicles
                ),
                key=lambda x: x[3],
            )
        ...
        self.load_to_json(
            "maneuver_challenges.json",
            self.car_with_maneuver_challenges,
            "maneuver_challenges",
        )
        self.load_to_json(
            "maneuver_challenge_record.json",
            self.maneuver_challenge_record,
            "maneuver_challenge_record",
        )
        self.load_to_json(
            "negligence_mode.json", self.negligence_mode, "negligence_mode"
        )
        self.load_to_json(
            "avoid_collision_mode.json",
            self.avoid_collision_mode,
            "avoid_collision_mode",
        )
        self.load_to_json(
            "accept_collision_mode.json",
            self.accept_collision_mode,
            "accept_collision_mode",
        )
        self.load_to_json("weight_record.json", self.weight_record, "weight_record")
        self.load_to_json(
            "comprehensive_info.json", self.comprehensive_info, "comprehensive_info"
        )
    ...
    def update_critical_moment_info(self, infos_dict):
        ...
        self.critical_moment_infos["criticality_step_info"].update(
            current_time, criticality_negligence
        )
        self.critical_moment_infos["ndd_step_info"].update(
            current_time, ndd_negligence_command
        )
        self.critical_moment_infos["drl_obs_step_info"].update(current_time, d2rl_obs)
        self.critical_moment_infos["drl_epsilon_step_info"].update(
            current_time, 1 - IS_prob
        )
        self.critical_moment_infos["real_epsilon_step_info"].update(
            current_time, 1 - IS_prob
        )
