...
class SafeTestNADE(SafeTestNDE):
    ...
    def on_step(self, ctx):
        ...
        ITE_control_cmds, veh_ctx_dicts, weight = self.NADE_decision(
    ...
    def apply_collision_avoidance(
        self, neglected_vehicle_list, ITE_control_command_dict
    ):
            ...
            for veh_id in neglected_vehicle_list:
                ...
                self.monitor.update_comprehensive_info(
                    timestamp,
                    {
                        "avoid_collision"
                        + veh_id: {
                            "avoid_collision_prob": avoid_collision_ndd_prob,
                            "IS_prob": IS_prob,
                            "weight": self.importance_sampling_weight,
                            "onestep_weight": avoid_collision_ndd_prob
                            / avoid_collision_IS_prob,
                        }
                    },
                )
        else:  # does not apply collision avoidance
            print(
                f"time: {timestamp}, neglected vehicle: {neglected_vehicle_list} accept collision"
            )
            for veh_id in neglected_vehicle_list:
                ...
                self.monitor.update_comprehensive_info(
                    timestamp,
                    {
                        ...
                        + veh_id: {
                            ...
                            "IS_prob": IS_prob,
                            "weight": self.importance_sampling_weight,
                            "onestep_weight": (1 - avoid_collision_ndd_prob)
                            / (1 - avoid_collision_IS_prob),
                        }
                    },
                )
    ...
    def get_maneuver_challenge_dicts(self, trajectory_dict, obs_dicts, veh_ctx_dicts):
        ...
        normal_future_trajectory_dict = {
            veh_id: trajectory_dict[veh_id].get("normal", None)
            for veh_id in trajectory_dict
        }
        negligence_future_trajectory_dict = {
            veh_id: trajectory_dict[veh_id].get("negligence", None)
            for veh_id in trajectory_dict
        }
        avoidance_future_trajectory_dict = {
            veh_id: trajectory_dict[veh_id].get("avoid_collision", None)
            for veh_id in trajectory_dict
        }
